...
class ComputerPlayer
  ...
  class SuperComputerPlayer
    def move(game, mark)
      ...
      node.children
      #!!!
      #need to use BFS instead of each
      node.children_arr.each do |node_child|
        if node_child.winning_node?(mark)
          return node_child.prev_move_pos
        end
      end
      node.children_arr.each do |node_child|
        return node_child if !node_child.losing_node?(mark)
      end
      raise "no winning or non-losing nodes"
    end
  end
end
